...
def create_book(request: Request, newBook: Book = Body(...)):
    book = jsonable_encoder(book)
    books = list(request.app.database["books"].find(limit=100))
    for book in books:
        if book['title'] == newBook.title and book['author'] == newBook.author:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"A book with the same {newBook.title} and {newBook.author} already exists")
        new_book = request.app.database["books"].insert_one(newBook)
